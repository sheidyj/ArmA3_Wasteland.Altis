...
private ["_nbUnits", "_box1", "_box2", "_para", "_p", "_paras", "_nil", "_veh", "_vel", "_class", "_marker", "_time", "_townName", "_missionPos", "_buildingRadius", "_putOnRoof", "_fillEvenly", "_tent1", "_chair1", "_chair2", "_cFire1"];

_setupVars =
{
	_missionType = "Military Base attack";
	_nbUnits = if (missionDifficultyHard) then { AI_GROUP_MEDIUM } else { AI_GROUP_MEDIUM };

	// settings for this mission
	_locArray = ((call MilitaryBaseattack) call BIS_fnc_selectRandom);
	...
	_buildingRadius = if (_buildingRadius > 350) then {(_buildingRadius*0.5)} else {_buildingRadius};
...
_setupObjects =
{
	...
	_box1 = createVehicle ["B_CargoNet_01_ammo_F", _missionPos, [], 10, "None"];
	_box1 setDir random 360;
	[_box1, "mission_USSpecial"] call fn_refillbox;
	
	_box2 = createVehicle ["B_CargoNet_01_ammo_F", _missionPos, [], 10, "None"];
	...
	[_aiGroup, _missionPos, _nbUnits] call MilitaryBaseattackGroup;
...
//	diag_log "[BRO'F] WASTELAND SERVER - Mission Military Base attack STARTED";
};
...
//	diag_log "[BRO'F] WASTELAND SERVER - Mission Military Base attack FAILED";
};
...
//	diag_log "[BRO'F] WASTELAND SERVER - Mission Military Base attack COMPLETED";
};
