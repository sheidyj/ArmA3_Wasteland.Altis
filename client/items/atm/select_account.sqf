...
private ["_accDropdown", "_curSel", "_dialog", "_deposit", "_depositall", "_withdraw", "_selAcc", "_accDropdown", "_feeText", "_totalText", "_players", "_oldPlayers", "_strPlayers", "_selData", "_selAcc", "_idx", "_data", "_selIdx", "_amount", "_fee", "_deposit", "_withdraw"];
...
if (!isNil "_selAcc" && {_selAcc != player}) then
{
	...
	if (ctrlShown _depositall) then { _depositall ctrlShow false };
}
else
{
	...
	if (!ctrlShown _depositall) then { _depositall ctrlShow true };
};
