...
//file rev by [IT42O]MjDoc
//file name APOC_cli_startAirdrop.sqf
...
{
	hint format["You don't have enough money in the bank account to request this airdrop!"];
	playSound "FD_CP_Not_Clear_F";
};
...
playSound3D ["a3\sounds_f\sfx\zoomin.wss",player,false,getPosASL player,1,1,25];									// by mjd
...
if ([parseText _confirmMsg, "Confirm", "OK", true] call BIS_fnc_guiMessage) then
{
	...
	playSound3D ["a3\dubbing_f_warlords\announcer\BLUFOR\airdrop.ogg",player,false,getPosASL player,1,1,25];		// by mjd
	sleep 2;
